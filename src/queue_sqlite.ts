...
export class SQLiteQueue implements QueueAdapter {
  ...
  async enqueue(request: TaskRequest): Promise<TaskRecord> {
    ...
    this.db.run(
      "INSERT INTO tasks (taskId, request, status, enqueuedAt) VALUES (?, ?, ?, ?)",
      [request.taskId, JSON.stringify(request), record.status, record.enqueuedAt]
    );
  ...
  async dequeue(): Promise<TaskRequest | undefined> {
    const row = this.db.query("SELECT taskId FROM queue ORDER BY id ASC LIMIT 1").get() as { taskId: string } | null;
    if (!row) return undefined;

    this.db.run("DELETE FROM queue WHERE taskId = ?", [row.taskId]);

    const taskRow = this.db.query("SELECT request FROM tasks WHERE taskId = ?").get(row.taskId) as { request: string } | null;
    return taskRow ? JSON.parse(taskRow.request) : undefined;
  }

  async getRecord(taskId: string): Promise<TaskRecord | undefined> {
    const row = this.db.query("SELECT * FROM tasks WHERE taskId = ?").get(taskId) as any;
    if (!row) return undefined;
